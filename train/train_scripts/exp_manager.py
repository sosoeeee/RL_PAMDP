...
from stable_baselines3 import HerReplayBuffer, HybridHerReplayBuffer
...
from train_scripts.callbacks import SaveVecNormalizeCallback, TrialEvalCallback
from train_scripts.hyperparams_opt import HYPERPARAMS_SAMPLER
from train_scripts.utils import ALGOS, get_callback_list, get_class_by_name, get_latest_run_id, get_wrapper_class, linear_schedule
...
class ExperimentManager:
    ...
    def __init__(
        ...
        self.wrapper_kwargs: Dict[str, Any] = {}
        ...
        self.config = str(default_path / f"hyperparams/{self.algo}.yml")
    ...
    def setup_experiment(self) -> Optional[Tuple[BaseAlgorithm, Dict[str, Any]]]:
        ...
        self._hyperparams = self._preprocess_action_noise(hyperparams, saved_hyperparams, env)   # useless when trainning with HPPO
    ...
    def read_hyperparameters(self) -> Tuple[Dict[str, Any], Dict[str, Any]]:
        ...
        if self.env_name in list(hyperparams_dict.keys()):
            ...
            raise ValueError(f"Hyperparameters not found for {self.algo}-{self.env_name} in {self.config}")
    ...
    def _preprocess_schedules(hyperparams: Dict[str, Any]) -> Dict[str, Any]:
        # Create schedules
        for key in ["learning_rate", "clip_range", "clip_range_vf", "delta_std", "ent_coef"]:
    ...
    def _preprocess_hyperparams(  # noqa: C901
        ...
        for kwargs_key in {"policy_kwargs", "replay_buffer_class", "replay_buffer_kwargs", "env_kwargs", "wrapper_kwargs"}:
        ...
        if "eval_env_name" in hyperparams.keys():
            self.eval_env_name = hyperparams["eval_env_name"]
            del hyperparams["eval_env_name"]
            if "eval_env_kwargs" in hyperparams.keys():
                self.eval_env_kwargs = hyperparams["eval_env_kwargs"]
                del hyperparams["eval_env_kwargs"]
            else:
                self.eval_env_kwargs = self.env_kwargs  # use the same env_kwargs for eval_env
                print('\n')
                print(f"Using {self.eval_env_name} for evaluation")
                print(f"Because no eval_env_kwags, using training env: {self.env_name} kwargs instead.")
                print('\n')
        else:
            self.eval_env_name = self.env_name
            self.eval_env_kwargs = self.env_kwargs                      # use the same env_kwargs for eval_env

        if "wrapper_kwargs" in hyperparams.keys():
            self.wrapper_kwargs = hyperparams["wrapper_kwargs"]
            del hyperparams["wrapper_kwargs"]
    ...
    def create_envs(self, n_envs: int, eval_env: bool = False, no_log: bool = False) -> VecEnv:
        ...
        if eval_env:
            spec = gym.spec(self.eval_env_name)
        else:
            spec = gym.spec(self.env_name)
        ...
        def make_env(**kwargs) -> gym.Env:
            # record video for evaluation env
            if eval_env and self.record_video:
                kwargs_ = {"render_mode": "rgb_array"}
                kwargs_.update(kwargs)
                env_ = spec.make(**kwargs_)
                return RecordVideo(env_, video_folder=self.save_path)
            else:
