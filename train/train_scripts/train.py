...
def train() -> None:
    ...
    parser.add_argument("--algo", help="RL Algorithm", default="hppo", type=str, required=False, choices=list(ALGOS.keys()))
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Record video of the agent during evaluation",
    )

    parser.add_argument(
        "--paraTag",
        help="Tag to distinguish different trainning parameters.",
        type=str,
        default=None,
    )

    ## ======================================= Weights and Biases related arguments ======================================= ##
    parser.add_argument(
        "--track",
        action="store_true",
        default=False,
    ...
    ## ======================================= Hyperparameter optimization related arguments ======================================= ##
    parser.add_argument(
    ...
    if args.track:
        ...
        # # run_name = f"{args.paraTag}_{args.env}__{args.algo}__{args.seed}__{int(time.time())}"
        # current_time = time.strftime("%Y%m%d-%H%M%S", time.localtime())
        # run_name = f"{args.paraTag}__{current_time}" if args.paraTag else f"{current_time}"
        ...
        try:
            ...
            raise ImportError(
                "if you want to use swanlab to track experiment, please install W&B via `pip install swanlab`"
            ) from e
